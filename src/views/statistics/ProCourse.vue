...
<div id="root">
	...
	<el-row>
		<el-col :span='24' class='toolbar' style='padding-bottom: 0px;'>
			<el-form :inline='true' :model='filters'>
				...
				<el-form-item>
					<el-input v-model='filters.course_id' placeholder="请输入课程 id"></el-input>
			  	</el-form-item>
				<el-form-item>
	...
	<el-table border :data="allData" highlight-current-row v-loading="initLoading" style="width: 100%;">
		...
		<el-table-column prop="fishNum" label="日课程销量（见习）" width="120" align='center' :formatter="formatNumber"></el-table-column>
		<el-table-column prop="newNum" label="日课程销量（新）" width="100" align='center' :formatter="formatNumber"></el-table-column>
		<el-table-column prop="oldNum" label="日课程销量（老）" width="100" align='center' :formatter="formatNumber"></el-table-column>
		<el-table-column prop="dayTotal" label="日课程销量（总计）" width="120" align='center' :formatter="formatNumber"></el-table-column>

		<el-table-column prop="fishRangeNum" label="累计课程销量（见习）" width="120" align='center' :formatter="formatNumber"></el-table-column>
		<el-table-column prop="newRangeNum" label="累计课程销量（新）" width="120" align='center' :formatter="formatNumber"></el-table-column>
		<el-table-column prop="oldRangeNum" label="累计课程销量（老）" width="120" align='center' :formatter="formatNumber"></el-table-column>
		<el-table-column prop="rangeTotal" label="累计课程销量（总计）" width="120" align='center' :formatter="formatNumber"></el-table-column>
	...
	methods: {
		...
		handleExport() {
			...
			let params = "?page_index=" + this.page_index + '&page_size=' + this.page_size + "&date=" + (this.filters.date ? Date.parse(this.filters.date) : '')
						...
						+ this.filters.rank_id + "&date_node=" + this.filters.date_node + "&course_id=" + this.filters.course_id + "&date_type=" + this.filters.date_type;
			// 借用隐藏的form表单提交数据 请求到excel进行下载
			this.$refs['oForm'].action = "/op/promoter/statistics/export/course" + params;
