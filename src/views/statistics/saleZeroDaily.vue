<template lang="html">
    <section>
        ...
        <el-table border :data="allData" highlight-current-row v-loading="initLoading" style="width: 100%;">
            ...
            <el-table-column prop="leaderType" label="职位" align='center'></el-table-column>
            <el-table-column prop="status" label="在职情况" align='center'></el-table-column>
            <el-table-column prop="enrollNumber" label="当期招生数" align='center'></el-table-column>
            ...
            <!-- <el-table-column prop="transRate" label="档期转化率" align='center'></el-table-column> -->
	...
	methods: {
		...
		handleExport() {
            let params = "?page_index=" + this.page_index + '&page_size=' + this.page_size + "&date=" + (this.filters.date ? Date.parse(this.filters.date) : '') +
				"&date_range_id=" + (this.filters.date_range_id ? this.filters.date_range_id.id : '');

            // 借用隐藏的form表单提交数据 请求到excel进行下载
			this.$refs['oForm'].action = "/op/promoter/stat_man/personal/zero_export" + params;
