        ...
        <section id="main">
            <el-card class="pieces" v-for="(item, index) in data" :key="item"  :body-style="{ padding: '0px' }"  @click.native='checkedPic(item)' @dblclick.native="checkedMaterial(item)">
                <div :class="item.conf.posterImgUrl ? 'ownImg' : 'defaultImg'">
                    <img :src="item.conf.posterImgUrl ? item.conf.posterImgUrl : '../../../static/images/defaultPostAudioImg.png'" alt="">
                </div>
	...
	data() {
		return {
			...
			playerOptions: {
				...
				sources: [{
					type: "audio/mp3",
					src: 'http://opbcvoqhz.bkt.clouddn.com/a19da144a7c492b91961ccb67746ecbbf98cacad.mp3',
				}],
				poster: "../../../static/images/defaultPostAudioImg.png",
				loop: false
			},
		}
	},
